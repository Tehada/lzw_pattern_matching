...
class pm_rmq : public rmq<iterator_type, value_type, difference_type> {
   ...
   */
  std::unique_ptr<sparse_rmq<typename std::vector<value_type>::const_iterator> >
      _super_rmq;
   ...
   */
  typedef naive_rmq<typename std::vector<value_type>::const_iterator>
      sub_block_rmq;
  ...
  static void normalize(iterator_type b, iterator_type e, OutputIterator o) {
 ...
 public:
  pm_rmq(iterator_type b, iterator_type e)
    ...
    std::for_each(
        boost::make_zip_iterator(boost::make_tuple(b, b + 1)),
    ...
    for (iterator_type block_begin = begin(); block_begin < end();
         block_begin += block_size()) {
      const iterator_type block_end =
          std::min(block_begin + block_size(), end());
      ...
      // found, construct one.
      std::unique_ptr<sub_block_rmq> &naive_ptr =
          _sub_block_rmqs[normalized_block];
      if (!naive_ptr) {
        naive_ptr.reset(new sub_block_rmq(normalized_block.begin(),
    ...
    _super_rmq.reset(
        new sparse_rmq<typename std::vector<value_type>::const_iterator>(
            _super_array_vals.begin(), _super_array_vals.end()));
  }
  ...
  difference_type query(iterator_type u, iterator_type v) const {
    ...
    const difference_type u_block_idx =
        difference_type(u - begin()) / block_size();
    const difference_type u_offset =
        difference_type(u - begin()) % block_size();
    const difference_type v_block_idx =
        difference_type(v - 1 - begin()) / block_size();
    const difference_type v_offset =
        difference_type(v - 1 - begin()) % block_size();
    ...
    if (block_diff == 0) {
      ...
      return (u_block_idx * block_size()) +
             u_naive.query_offset(u_offset, v_offset + 1);

    } else {
      const iterator_type u_block_end =
          std::min(end(), begin() + ((u_block_idx + 1) * block_size()));
      ...
      const difference_type u_min_idx =
          (u_block_idx * block_size()) +
          u_naive.query_offset(
              u_offset, u_block_end - (begin() + (block_size() * u_block_idx)));
      const difference_type v_min_idx =
          (v_block_idx * block_size()) + v_naive.query_offset(0, v_offset + 1);
      ...
      if (block_diff == 1) {
        ...
        const difference_type super_idx =
            _super_rmq->query(_super_array_vals.begin() + u_block_idx + 1,
        ...
        if (u_min_val < v_min_val) {
          return u_min_val < _super_array_vals[super_idx]
                     ? u_min_idx
                     : _super_array_idxs[super_idx];
        } else {
          return v_min_val < _super_array_vals[super_idx]
                     ? v_min_idx
                     : _super_array_idxs[super_idx];
        }
      }
